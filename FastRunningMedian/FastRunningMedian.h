 ...
 * FastRunningMedian.h
 *
 * Created: 28/10/2014 14:59:31
 *  Author: Paul Soulsby
 */ 


#ifndef FASTRUNNINGMEDIAN_H_
#define FASTRUNNINGMEDIAN_H_
//
...
template <typename T, uint8_t N, T default_value> class FastRunningMedian {
	...
	void initBuffers(T new_value){
		// Init buffers
		uint8_t i = _window_size;
		while( i > 0 ) {
			i--;
			_inbuffer[i] = new_value;
			_sortbuffer[i] = new_value;
		}		
	}
