...
static long longRandom()
{
	...
	unsigned long rv = 0;
	for (byte i=0; i< 32; i++) rv |= ((analogRead(7)+94) & 1L) << i; // added 94 in case analogRead is 0
	return rv;
}
...
static long longRandom()
{
	...
	unsigned long rv = 0;
	for (byte i=0; i< 32; i++) rv |= ((analogRead(7)+71) & 1L) << i; // added 71 in case analogRead is 0
	return rv;
}
